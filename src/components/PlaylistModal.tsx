...
export type UserPrivatePlaylistsResponseTypes = {
  playlists: Playlists[];
  pagination: {
    page: number;
    limit: number;
...
const PlaylistModal: React.FC<PlaylistModalProps> = ({
  ...
  const handleTogglePlaylist = useCallback(
    async (playlistId: number, isInPlaylist: boolean) => {
      try {
        if (isInPlaylist) {
    ...
    [addVideoToPlaylistMutation, removeVideoFromPlaylistMutation, videoId]
  );
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Save to...">
      ...
      <CreatePlaylistButton
        ...
        disabled={playlistsData?.playlists?.length! >= 10}
      />
...
const EmptyPlaylistMessage: React.FC<{ onCreateNew: () => void }> = ({
  onCreateNew,
}) => (
  <div className="flex flex-col md:flex-row items-center justify-center gap-2">
    <p>You don't have any playlists yet.</p>
    <button onClick={onCreateNew} className="text-primary hover:underline">
      Create New
    </button>
  </div>
);
...
const PlaylistItem = React.memo(
  ({ playlist, videoId, onToggle }: PlaylistItemProps) => {
